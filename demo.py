    ...
    if isinstance(obj, dict):
        for k, v in obj.items():
            if isinstance(v, dict):
                id_key = next((x for x in v if x.endswith("_id")), "id")
                print(f"        {k:<26} = <nested id={v.get(id_key, '?')}>")
            else:
                print(f"        {k:<26} = {v!r}")
    else:
        try:
        ...
        for k, v in fields.items():
            if hasattr(v, "__slots__"):
                try:
                    inner_id = next(getattr(v, s) for s in v.__slots__ if s.endswith("_id"))
                    print(f"        {k:<26} = <{type(v).__name__} id={inner_id}>")
